...
function normalizeCharge(saved) {
  ...
  return {
    ...
    intent:
      saved.intent === -1 || saved.intent === 'careless'
        ? 'careless'
        : 'aware',

    finish:
      saved.finish === -1 || saved.finish === 'stopped'
        ? 'stopped'
        : 'complete',

    ready:
      saved.ready === -1 || saved.ready === 'affect'
        ? 'affect'
        : saved.ready === 1 || saved.ready === 'conspiracy'
          ? 'conspiracy'
          : saved.ready === 'other'
            ? 'other'
            : 'planned',
    ...
    reason:
      saved.reason === -9 || saved.reason === 'necessity-none'
        ? 'necessity-none'
        : saved.reason === -1 || saved.reason === 'necessity'
          ? 'necessity'
          : saved.reason === 1 || saved.reason === 'official'
            ? 'official'
            : saved.reason === 'assistance'
              ? 'assistance'
              : 'none',
...
let state = loadArray('kz-case')
  .map(normalizeCharge)
  .filter(Boolean);

let filter = 0;
let baseTerm = clamp(
  Number(localStorage.getItem('kz-base-term')) || 15,
  15,
  45
);
...
let lastOpened = '';
let layoutMode =
  localStorage.getItem('kz-layout') === 'stack'
    ? 'stack'
    : 'split';
...
function matches(article, query) {
  const haystack = normalizeText(
    `${article.code} ${article.name} ${article.description} ${typeNames[article.type - 1]}`
  );
...
function renderFilters() {
  const options = [
    ...
    [
      'selected',
      `Выбранные${state.length ? ` · ${state.length}` : ''}`
    ]
  ];
  ...
  $('#filters').innerHTML = options
    .map(([value, label]) => {
      ...
      return `
        <button
          class="filter-button${value === 'selected' ? ' selected-filter' : ''}${active ? ' active' : ''}"
          type="button"
          data-filter="${value}"
          aria-pressed="${active}"
        >
...
function articleButton(article) {
  const selected = state.some(
    charge => charge.code === article.code
  );

  return `
    <button
      class="article-card${selected ? ' selected' : ''}"
      type="button"
      data-code="${article.code}"
      aria-pressed="${selected}"
      title="${esc(article.description)}"
      ${severityStyle(article.level)}
    >
...
function renderMatrix() {
  const headings = levelLabels
    .slice(1)
    .map((label, index) => `
      <div
        class="severity-heading"
        ${severityStyle(index + 1)}
      >
        <b>${label}</b>
        <span>${levelNames[index + 1]}</span>
      </div>
    `)
    .join('');

  const rows = articleNames
    .map((row, rowIndex) => {
      const cells = row
        .map((_, levelIndex) => {
          const article = articles.find(
            item =>
              item.code === `${rowIndex + 1}${levelIndex + 1}`
          );

          return articleButton(article);
        })
        .join('');
  ...
  return `
    <div class="law-matrix">
      <div class="matrix-corner">Тип</div>
      ${headings}
      ${rows}
    </div>
  `;
...
function renderSearchResults(query) {
  const visible = articles.filter(article => {
    if (
      filter === 'selected' &&
      !state.some(charge => charge.code === article.code)
    ) {
      return false;
    }
  ...
  if (!visible.length) {
    return `
      <div class="no-results">
        ...
        <span>Попробуйте другой номер или формулировку.</span>
      </div>
    `;
  }
    ...
    <div class="results-mode">
      ${visible
        .map(article => {
          ...
          return `
            <button
              class="result-article${selected ? ' selected' : ''}"
              type="button"
              data-code="${article.code}"
              aria-pressed="${selected}"
              ${severityStyle(article.level)}
            >
              ...
              <span>
                <strong>${esc(article.name)}</strong>
...
function modifierDelta(charge) {
  const intent =
    charge.intent === 'careless'
      ? -1
      : 0;

  const finish =
    charge.finish === 'stopped'
      ? -1
      : 0;
  ...
  const ready =
    charge.ready === 'affect'
      ? -1
      : charge.ready === 'conspiracy'
        ? 1
        : 0;

  const reason =
    charge.reason === 'necessity'
      ? -1
      : charge.reason === 'necessity-none'
        ? -99
        : charge.reason === 'official'
          ? 1
          : 0;
...
function effectiveLevel(charge) {
  if (charge.reason === 'necessity-none') {
    return 0;
  }

  return clamp(
    charge.level +
      modifierDelta(charge) +
      Math.max(0, charge.repeat - 1),
    1,
    5
  );
}
...
function levelShiftMarkup(charge) {
  ...
  if (!effective) {
    return '<span class="level-shift changed">снята</span>';
  }

  const changed = effective !== charge.level;

  return changed
    ? `<span class="level-shift changed">X${charge.level} → X${effective}</span>`
    : '';
}
  ...
  $('#charges').innerHTML = state
    .map(charge => `
      <details
        ...
        ${charge.code === lastOpened ? 'open' : ''}
        ${severityStyle(charge.level)}
      >
        <summary>
          ...
          <span class="charge-name">
            <b>${esc(charge.name)}</b>
            <small>${levelNames[charge.level]} нарушение</small>
          </span>
          ...
          <button
            class="remove"
            type="button"
            data-remove="${charge.code}"
            aria-label="Удалить статью ${charge.code}"
            title="Удалить"
          >
            ×
          </button>
        ...
        <div class="charge-body">
          ...
          <div class="mods">
            ...
            <div class="mod-row">
              ...
              ${buttonGroupMarkup('finish', charge.finish)}
            </div>

            <div class="mod-row">
              <span>Готовность</span>
              ${buttonGroupMarkup('ready', charge.ready)}
            </div>
          ...
          <div class="repeat-control">
            <label>
              Нарушений этой статьи
              <span>Повтор повышает тяжесть</span>
            </label>
            ...
            <div class="stepper">
              ...
              <input
                ...
                max="5"
                value="${charge.repeat}"
                aria-label="Количество нарушений статьи ${charge.code}"
              >
...
function updateChargeLevels() {
  state.forEach(charge => {
    const details = document.querySelector(
      `.charge[data-code="${charge.code}"]`
    );

    const oldBadge = details?.querySelector('.level-shift');

    if (oldBadge) {
      oldBadge.outerHTML = levelShiftMarkup(charge);
    }
  });
}
...
function calculate() {
  const active = state
    .map(charge => ({
      ...charge,
      effective: effectiveLevel(charge)
    }))
    .filter(charge => charge.effective)
    .sort((a, b) => b.effective - a.effective);

  const byTypeMap = new Map();

  active.forEach(charge => {
    if (!byTypeMap.has(charge.type)) {
      byTypeMap.set(charge.type, charge);
    }
  });
  ...
  const highest = Math.max(
    0,
    ...byType.map(charge => charge.effective)
  );

  const main = byType.find(
    charge => charge.effective === highest
  );

  const additions =
    highest === 3 || highest === 4
      ? byType
          .filter(charge => charge !== main)
          .reduce(
            (total, charge) =>
              total +
              ([0, 0, 5, 10, 15, 0][charge.effective] || 0),
            0
          )
      : 0;
  ...
  if (state.length && !highest) {
    ...
    disciplinary =
      'Предупреждение и профилактическая беседа';
  } else if (highest === 2) {
    result = 'Без заключения';
    status = 'Лёгкое нарушение';
    disciplinary =
      'Объяснительная и профилактическая беседа';
  } else if (highest === 5) {
  ...
  const rows = [
    main &&
      `
        <div class="breakdown-row">
          <span>Основная статья</span>
          <span>${main.code} · X${main.effective}</span>
        </div>
      `,
    ...
    additions > 0 &&
      `
        <div class="breakdown-row">
          <span>Дополнительные статьи</span>
          <span>+${additions} минут</span>
        </div>
      `,
      ...
      `
        <div class="breakdown-row">
          ...
          <span>
            ...
            ${
              ignored === 1
                ? 'статья не суммируется'
                : 'статьи не суммируются'
            }
          </span>
        </div>
      `,
    ...
    total >= 40 &&
      ...
      `
        <div class="warning">
          Срок 40+ минут: допускается отбывание наказания
          в пермабриге.
        </div>
      `
  ].filter(Boolean);
...
$('#articles').addEventListener('click', event => {
  ...
  if (button) {
    toggleArticle(button.dataset.code);
  }
});
...
$('#charges').addEventListener('click', event => {
  ...
  if (step) {
    ...
    updateCharge(
      ...
      'repeat',
      charge.repeat + Number(step.dataset.step)
    );
...
$('#charges').addEventListener(
  'toggle',
  event => {
    if (
      event.target.matches('.charge') &&
      event.target.open
    ) {
      lastOpened = event.target.dataset.code;
    }
  },
  true
);
...
$('#baseTerm').addEventListener('input', event => {
  const min = Number(event.target.min) || 15;
  const max = Number(event.target.max) || 45;
